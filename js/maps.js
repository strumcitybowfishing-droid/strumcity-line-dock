...
function showStaticMap(loc) {
  ...
  // Use official TWDB bathymetric contour maps (depth charts) for original TX lakes or generated OpenTopoMap topo/contour/relief JPGs
  // (via scripts/build-map-backgrounds.py --style topo) for expanded locations (new TX + AR + TN/AL lakes).
  // Gives the "cool topographical fishing map" / Garmin-style look with contours. Marine + trinity use satellite.
  // Falls back to Leaflet (satellite + topo overlay) if the static jpg is missing.
  let mapFile = `${loc.id}.jpg`;
  if (loc.type === "weather" && loc.id !== "trinity") {
    mapFile = `${loc.id}-depth.jpg`;
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    img.alt = loc.type === "weather" && loc.id !== "trinity"
      ? `Depth chart / topographical fishing map — ${loc.label}`
