...
function showStaticMap(loc) {
  ...
  // Use official TWDB bathymetric contour maps (depth charts from hydrographic surveys) as backgrounds for lake pages.
  // These are the real "fishing nav map" / depth chart style like Garmin plotters (contour lines with depths in feet).
  // Generated by rendering TWDB public PDFs (e.g. Conroe 2020 5ft contours, etc.) to high-res JPGs.
  // For lakes (weather type, except river trinity). Marine (surfside) and trinity keep satellite.
  // Falls back to Leaflet (satellite + topo overlay) if missing.
  ...
  return new Promise((resolve, reject) => {
    ...
    img.alt = loc.type === "weather" && loc.id !== "trinity"
      ? `Depth chart / fishing nav map (TWDB bathymetry) — ${loc.label}`
